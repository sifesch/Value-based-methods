...
def create_training_plot(scores: list, trial_num: str = '01'):
    ...
    # Plot horizontal line for the Target Score
    plt.axhline(y=13, color='r', linestyle='--', linewidth=2, label="Target Score")
    plt.ylabel('Score')
    plt.xlabel('Episode #')
    plt.title('Training Progress')
    plt.legend()
    plt.savefig(f'results/training_scores_trial_{trial_num}.png', bbox_inches='tight', transparent=True, facecolor='white')
    plt.show()
...
def create_visual_network_architecture_png(state_size: int= 37, fc1_units: int = 64, fc2_units: int = 64, action_size: int = 4):
    ...
    plt.savefig("results/model_architecture.png", bbox_inches="tight")
    plt.show()
